...
`cargo test --all` runs **1410 passing** (core 342, dpop 21, delegation 44,
gateway 774, cloud 228, umbrella 1, by `cargo test -p <crate>`), which is the figure the README badge
states and `scripts/stated-numbers.sh` gates (invariant 12). Core grew from 340 to
342 and gateway from 765 to 774 on 2026-09-24 (invariant 61, W2a): two in
`core::taint` for `declared_tool_defs_in` (both wire shapes, a 200-seed hostile
sweep); three in `gateway::defaults` for `TOKENFUSE_TOOLS_PRUNE`'s parsing; six
in `crates/gateway/tests/shadow_tool_pruning.rs` for the shadow measurement
wired into `proxy::messages` (one guard, `off_makes_no_filter_call`, and five
run red against the unfixed wiring first).
