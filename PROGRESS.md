...
**Last updated:** 2026-07-03 (mobile companion: A8 — device pairing + Secure-Enclave-signed mutations)
...
| Mobile companion (TokenFuse Pocket, iOS) | 🔨 in progress | Native SwiftUI command center: live burn rate in the Dynamic Island, push on anomalies, **Face-ID kill / budget with Secure-Enclave-signed mutations**. Full execution plan (ADRs, wire protocols, PR-by-PR breakdown) in [docs/14-mobile-companion.md](docs/14-mobile-companion.md). Phase A ports the control plane Go→Rust (`crates/cloud`, ADR-7) and adds pairing / signed mutations / SSE / OpenAPI / APNs — **no Xcode required**; Phase B is the iOS app; Phase C ships to the App Store. Dev host verified ready 2026-07-03 (Xcode 26.6, iOS 26.5 SDK + simulators, Swift 6.3.3, XcodeGen 2.45.4). **Done:** A1 (plan + ADR-7, #61); A2 (`crates/cloud` skeleton — health + ingest + in-memory store port, #62); A3 (read endpoints `/v1/runs` `/v1/summary` `/v1/alerts` + durable JSON snapshot/autosave + CORS, restart-persistence verified live, #63); A4 (mutations `kill`/`budget` + `/v1/kills` `/v1/budgets` + RBAC admin-only 403/viewer, #64); A5 (**cutover** — dashboard served from `crates/cloud`, new Rust Dockerfile, release.yml/compose/CI swapped, **Go `cloud/control-plane` deleted**, docs updated, 21 tests, #65 — **Go is gone from the project**). A6 (OpenAPI 3.1 via utoipa — `GET /openapi.json` + `--openapi` dump + CI `openapi` job validates/uploads; #66); A7 (**live data**: SSE `/v1/stream` + burn-rate `/v1/series`; #67); A8 (**hardware-backed kill switch**: device pairing via one-time code + `POST /v1/pair{/new}`, and kill/budget accept Secure-Enclave **ES256-signed** requests — P-256/SHA-256, replay + stale-ts + nonce-LRU defense, `403 signature_invalid`; devices persist; 36 tests incl. 5 e2e with a real P-256 key; #68). **Next:** A9 (APNs push via crate `a2` + Live Activity — last server step). After A9 Phase A is done → Phase B (iOS app; needs Apple Developer + device). |
