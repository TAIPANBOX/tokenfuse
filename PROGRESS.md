...
| Gateway → Cloud telemetry (`CloudSink`) | ✅ done | `crates/gateway/src/cloudsink.rs`: batches settled `CallRecord`s and POSTs them async (periodic flush; since #295 every record carries the unit's `owner`, invariant 54) to the control plane; `TOKENFUSE_CLOUD_URL` + `TOKENFUSE_CLOUD_KEY`, composed via `TeeSink`. `CallRecord` gained `Serialize`. Verified end-to-end: 3 calls → Cloud shows 3 runs / $0.0315. `cloud/docker-compose.yml` runs the whole stack (`docker compose up`). |
...
`cargo test --all` runs **1370 passing** (core 340, dpop 21, delegation 44,
gateway 736, cloud 228, umbrella 1, by `cargo test -p <crate>`), which is the figure the README badge
states and `scripts/stated-numbers.sh` gates (invariant 12). Gateway grew from 731 to 736 and cloud from 224 to 228 on 2026-09-18 (invariant 54, tokenfuse#295): three in `identitymap::tests` and two in `cloudsink::tests` for the owner on the wire, four in `cloud::store` for the owner fold. Cloud grew from 201 to 223 on
...
Before that, gateway grew from 684 to 705 on 2026-09-18 (invariant 50, after #292 and
...
`fable_missed.rs`), eight new in `tokenfuse-gateway` (seven in `proxy::tests`,
one in `unitledger::tests`); `crates/gateway/tests/cluster_backend.rs` gained
two (`--features cluster`, not part of the default-feature count above). The
twelfth `ledger::tests` addition, one of the seven `proxy::tests` additions and
one of the two `cluster_backend.rs` additions landed after a second-model
review of the first commit found one untested line and eight smaller gaps.
dpop grew from
