...
![tests](https://img.shields.io/badge/tests-1500-brightgreen)
...
> ```bash
> docker run -p 4100:4100 -e TOKENFUSE_ALLOW_STUB=1 ghcr.io/taipanbox/tokenfuse:v1.1.0
> ```
...
- 📨 **Agent-event NDJSON export** *(opt-in)*: set `TOKENFUSE_EVENTS_PATH=<file>` and every breaker trip, DLP/taint block, and MCP rug-pull is appended as one NDJSON line in the shared [Agent Passport](https://github.com/TAIPANBOX/agent-passport) event envelope (`taipanbox.dev/agent-event/v0.2`). Unset by default: zero hot-path cost, no file handle opened. A path that cannot be opened at startup (a missing directory, a directory the process may not write to) costs one line at warn naming the path and the error and saying the export is off; the process keeps running, in both the gateway and the control plane. Writes are fail-open (a write error is logged, never a request failure), an event with no identity is skipped and counted, never fabricated, and a chain a delegation token PROVED carries the token that proved it (SPEC 5.2 `delegation_proof`) and names the agent the record is filed under when no `x-fuse-agent-id` header was sent. Every line carries the spec's §6.5 `prev_hash` integrity chain (one file = one chain, resumed across restarts); verify a stream with `agent-conform -chain <file>`. Tamper-evidence, not tamper-proof: a partial edit or truncation no longer passes silently.
- 🐍 **Python SDK**, sub-µs decision path, four public container images, and published npm / crates.io / PyPI packages.

**Agent Passport.** TokenFuse's `x-fuse-agent-id` / `x-fuse-run-id` / `x-fuse-parent-run-id` headers, the new `x-fuse-on-behalf-of` delegation-chain header (a comma-separated, ordered, root-first list of `agent://` / `user://` URIs, captured to the trace verbatim and never truncated when forwarded; two caps, and they bound different things: a raw header over 4 KiB is ignored as if absent, warned and counted, and a chain of more than 32 entries, the spec's maximum depth, is refused with `400` and one `identity_mismatch` event whether or not a delegation issuer is configured, since the shared event envelope refuses a longer chain; the run-ancestry walk over `x-fuse-parent-run-id` has its own, separate cap of 64 ancestors across requests, refused rather than truncated, which bounds which runs a call rolls up into, not whom it acts for), `parent_run_id` (now its own persisted Parquet trace column, not just an in-memory budget-hierarchy key), and the agent-event envelope above all follow one shared spec - [Agent Passport](https://github.com/TAIPANBOX/agent-passport) - used across the TAIPANBOX agent-governance stack (TokenFuse for spend, Wardryx for policy, Engram for memory, Idryx for access, Qryx for crypto, Verdryx for quality, Mockryx for pre-prod, heraldyx for the mail), so the same agent identifier and delegation chain read the same way in every product's traces and events. Idryx, specifically, ingests TokenFuse's agent-events as a behavioral source for identity-graph correlation.
...
docker run -p 4100:4100 \
  ...
  ghcr.io/taipanbox/tokenfuse:v1.1.0
```
...
gh attestation verify tokenfuse-x86_64-unknown-linux-musl -R TAIPANBOX/tokenfuse
```

The image:

```bash
cosign verify ghcr.io/taipanbox/tokenfuse:v1.1.0 \
  --certificate-identity-regexp '^https://github.com/TAIPANBOX/tokenfuse/\.github/workflows/release\.yml@refs/tags/v' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
gh attestation verify oci://ghcr.io/taipanbox/tokenfuse:v1.1.0 -R TAIPANBOX/tokenfuse
```

Releases before v0.5.0 carry checksums only. Verified on 2026-09-13 from a machine that built nothing: `sha256sum -c` OK on a downloaded binary, and `gh attestation verify` on the binary and on the image both returned a SLSA provenance v1 statement signed by this workflow at `refs/tags/v0.5.0`. The release assets are plain binaries, not archives: `tokenfuse-<target>` and `tokenfuse-cloud-<target>`.
...
A third, experimental since 2026-09-24: an access token that [vouchryx](https://github.com/TAIPANBOX/vouchryx) issued through Cross App Access, where an enterprise identity provider's assertion about a person (an ID-JAG, `draft-ietf-oauth-identity-assertion-authz-grant-04`) is redeemed for a five-minute token scoped to this broker, presented as `Authorization: Bearer`. Set `TOKENFUSE_MCP_ACCEPT_XAA=on` and `TOKENFUSE_MCP_RESOURCE` (this broker's own resource URL, https, which the token's audience must equal exactly), with `TOKENFUSE_DELEGATION_ISSUER` and `TOKENFUSE_DELEGATION_JWKS` naming vouchryx and `TOKENFUSE_DELEGATION_URL` the broker's public origin (the delegation door in the same process requires it). The broker then publishes [RFC 9728](https://www.rfc-editor.org/rfc/rfc9728) metadata at `/.well-known/oauth-protected-resource`, every refusal points at it in `WWW-Authenticate`, a call is attributed to the agent the token names with the person as the root of its chain, and a token for another resource, a token bound to a key, a delegation token presented as bearer, or a token naming a revoked person or agent is refused. With XAA on, a caller presenting no credential is refused even when no other door is configured. The grant is an IETF draft, so both settings are experimental in `compat/1.0.json`; CLAUDE.md invariant 62 holds the rules.
...
```bash
docker run -p 4100:4100 -e TOKENFUSE_ALLOW_STUB=1 ghcr.io/taipanbox/tokenfuse:v1.1.0   # gateway, offline
cd cloud && docker compose up                                                    # + Cloud dashboard (:3000)
```

Images on GHCR: `tokenfuse` · `tokenfuse:<tag>-cluster` · `tokenfuse-control-plane` · `tokenfuse-dashboard`, every one pinned by tag (no `:latest`).
...
**Is it production-ready?** It is 1.0.x: the surface is written and held (`COMPATIBILITY.md`), it is CI-tested and it was run on real boxes, clusters and providers before the tag (`docs/releases/v1.0.0.md`), but it is not audited or battle-hardened. Start in shadow mode and evaluate.
