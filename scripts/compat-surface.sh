...
# 1.0 tags (agent-passport and agent-stack-go on 2026-09-12, then trailryx,
# idryx, qryx, wardryx and scopyx on 2026-09-13) each came with the surface
# written down and a gate that fails when it moves; this is the same pair for
# this repository, written in its 1.0.0 release commit. estate-gates C19 asks
# whether this file and the manifest exist and CI runs this; this file asks
...
# literal ("name", 'name' or `name`), so a comment mentioning it does not
# count; an `lhs=rhs` entry must appear as an assignment `lhs = rhs`,
# whitespace free. Then COMPATIBILITY.md must equal the rendering of the
# manifest; `--write` regenerates it, so the human form is never typed twice.
#
...
#
# That the code does what the name says: `/v1/messages` present in the router
# is not `/v1/messages` answering. That a name absent from the manifest is not
# part of the surface: additive things may exist unlisted, by design. That a
# route still carries the same method, that a header still means the same
# thing, that a Parquet column kept its type: the tests behind invariants 2, 6
# and 14 hold those. The `additive` and `experimental` lists are documentation
# for a reader and are rendered, never checked.
set -uo pipefail
