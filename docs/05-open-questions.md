...
1. **License, final call:** Apache-2.0 (maximum adoption) vs AGPL (protection from cloud clones). Leaning toward Apache-2.0.
...
- [x] Naming research → chose `tokenfuse` (2026-07-02).
- [x] `git init` + first commit + public GitHub repo.
- [ ] Reserve the name: register `tokenfuse` on crates.io/npm/PyPI and the `tokenfuse.dev` domain before launch.
- [ ] Phase 0: break the 3 spikes into tickets with acceptance criteria:
   ...
   - propagation of X-Fuse headers through the Claude Agent SDK and LangGraph.
- [ ] Repo skeleton: `crates/gateway`, `crates/policy`, `crates/ledger`, `crates/tui`, `dashboard/`.
